const PROMPT = 'Extract the recipe. Keep the recipe name and instructions in their original language — do NOT translate them. Return ONLY a JSON object with: {"name": "recipe name in original language", "category": "category or null", "servings": number or null, "ingredients": [{"name": "ingredient", "quantity": number or null, "unit": "unit or null"}], "instructions": "step by step instructions as plain text in original language, each step on a new line, or null if not available"}. No markdown, no extra text. If no recipe found, return {"error": "no recipe found"}.'
...
async function extractFromUrl(url) {
  ...
  console.log(`[extract-recipe] fetch status: ${fetchRes.status}`)
  ...
  console.log(`[extract-recipe] HTML length: ${html.length}`)

  // Try JSON-LD structured data first
  const jsonLdMatches = html.match(/<script[^>]*type="application\/ld\+json"[^>]*>([\s\S]*?)<\/script>/gi) || []
  console.log(`[extract-recipe] JSON-LD blocks found: ${jsonLdMatches.length}`)
  ...
  for (const match of jsonLdMatches) {
    try {
      ...
      if (recipe) {
        console.log(`[extract-recipe] JSON-LD recipe found: "${recipe.name}" ingredients=${recipe.recipeIngredient?.length ?? 0} hasInstructions=${!!recipe.recipeInstructions}`)
      ...
      console.log(`[extract-recipe] JSON-LD parse error: ${e.message}`)
    }
  }
  ...
  const stripped = html.replace(/<script[\s\S]*?<\/script>/gi, '')
    ...
    .replace(/\s+/g, ' ')
    .slice(0, 6000)
  console.log(`[extract-recipe] falling back to Claude, stripped length: ${stripped.length}`)
  ...
  console.log(`[extract-recipe] Claude fallback response length: ${text?.length ?? 0}`)
...
export default async function handler(req, res) {
  ...
  console.log(`[extract-recipe] called — mode: ${image ? 'image' : url ? 'url' : 'text'}${url ? ` url=${url}` : ''}`)
  ...
  try {
    ...
    if (url) {
      ...
      let userContent
      if (image) {
        console.log(`[extract-recipe] image size: ${image.length} chars base64`)
        userContent = [
          { type: 'image', source: { type: 'base64', media_type: 'image/jpeg', data: image } },
          { type: 'text', text: PROMPT }
        ]
      } else {
        console.log(`[extract-recipe] text length: ${text.length}`)
    ...
    console.log(`[extract-recipe] success — name="${result.name}" ingredients=${result.ingredients?.length ?? 0} hasInstructions=${!!result.instructions}`)
