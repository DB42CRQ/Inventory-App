...
export default function AuthPage() {
  ...
  async function handleForgot(e) {
    ...
    const { error } = await supabase.auth.resetPasswordForEmail(form.email, {
      redirectTo: 'https://inventory42.com/reset-password'
    })
    if (error) setError(translateError(error.message, t))
  ...
  // Bestätigung nach Passwort-Reset
  if (resetSent) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-primary-50 to-indigo-100 flex items-center justify-center p-4">
        <div className="bg-white rounded-3xl shadow-xl w-full max-w-sm p-8 text-center">
          ...
          <h2 className="text-lg font-bold text-gray-900 mb-2">
            {t.resetEmailSentTitle ?? 'E-Mail gesendet!'}
          </h2>
          <p className="text-sm text-gray-500 mb-1">
            {t.resetEmailSentText ?? 'Wir haben einen Link zum Zurücksetzen des Passworts an'}
          </p>
          <p className="text-sm font-medium text-gray-800 mb-4">{form.email}</p>
          <p className="text-sm text-gray-500 mb-6">
            {t.resetEmailSentText2 ?? 'gesendet. Bitte prüfe dein Postfach.'}
          </p>
          <Button variant="secondary" className="w-full"
        ...
        {mode === 'forgot' ? (
          <form onSubmit={handleForgot} className="flex flex-col gap-4">
            <div className="text-center mb-2">
              <h2 className="font-semibold text-gray-900">{t.forgotTitle ?? 'Passwort zurücksetzen'}</h2>
              <p className="text-sm text-gray-500 mt-1">
                {t.forgotHint ?? 'Gib deine E-Mail ein — wir schicken dir einen Link.'}
              </p>
            </div>
            ...
            <Button type="submit" size="lg" disabled={loading}>
              {loading ? t.loading : t.forgotSend ?? 'Link senden'}
            </Button>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            {mode === 'login' && (
              <button type="button" onClick={() => { setMode('forgot'); setError('') }}
                className="text-xs text-gray-400 hover:text-primary-500 text-center transition-colors">
